"""Train ECGWindowTransformer on [t-24h, t-12h] ECG waveforms only; supervise anchor s2f/p2f change."""
...
from ecg_window_dataset import ECGWindowDataset  # noqa: E402

_EX = Path(__file__).resolve().parent
sys.path.insert(0, str(_EX))
from common import (  # noqa: E402
    collate_ecg_window_batch,
    eval_loader,
    forward_loss_from_logits,
    stratify_labels_from_anchor,
)
from model import ECGWindowTransformer  # noqa: E402
...
def main(args):
    ...
    print(f"ECGWindowTransformer  device={device}")

    ecg_ckpt = args.ecg_ckpt_path.strip() or None

    full_ds = ECGWindowDataset(
    ...
    for epoch in range(args.epochs):
        ...
        st = eval_loader(model, val_loader, device, "ecg_seq", "ecg_mask")
        print(
    ...
    test_st = eval_loader(model, test_loader, device, "ecg_seq", "ecg_mask")
    print(
    ...
    with open(out_dir / "results.json", "w") as f:
        json.dump(
            {
                "task": "ecg_window_transformer_anchor_s2f_p2f",
...
if __name__ == "__main__":
    ...
    p.add_argument("--ecg_ckpt_path", default=ECG_CKPT_DEFAULT)
    ...
    p.add_argument(
        "--output_dir",
        default=str(Path(__file__).resolve().parent / "output_ecg_window"),
    )
