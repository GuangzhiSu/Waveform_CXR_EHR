"""Test/val classification reports for CXREncoderTransformer (s2f / p2f heads)."""
...
@torch.no_grad()
def collect_head_preds_cxr(model, loader: DataLoader, device: torch.device, head: str) -> tuple:
    labels: list = []
    preds: list = []
    for batch in loader:
...
def evaluate_split_both_heads(model, loader: DataLoader, device: torch.device, split_name: str, num_classes: int) -> dict:
    ...
    out = {}
    for head in ("s2f", "p2f"):
        y_true, y_pred = collect_head_preds_cxr(model, loader, device, head)
