...
def forward_loss(model: EHREncoderTransformer, batch: dict) -> torch.Tensor:
    total, _ = forward_loss_parts(model, batch)
    return total


def forward_loss_parts(model: EHREncoderTransformer, batch: dict) -> tuple:
    ...
    loss_s = masked_ce(log_s2f, s_tgt, s_ok)
    loss_p = masked_ce(log_p2f, p_tgt, p_ok)
    return loss_s + loss_p, {"loss_s2f": loss_s, "loss_p2f": loss_p, "log_s2f": log_s2f, "log_p2f": log_p2f}
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            b = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
            loss, parts = forward_loss_parts(model, b)
            opt.zero_grad()
            loss.backward()
