"""Train temporal EHR ARDS severity classification baseline (average pooling only)."""
import argparse
import json
import sys
...
from EHRUni.model import EHRClassificationAverageBaseline
...
def main(args):
    ...
    print(f"EHR ARDS Classification (average pooling only). Device: {device}")
    ...
    model = EHRClassificationAverageBaseline(
        input_dim=input_dim,
    ...
    results = {
        ...
        "modality": "EHR-only temporal (average pooling only)",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default="./output_average")
