...
```bash
sbatch CXREncoderTransformer/run_train.sh --max_samples 5000 --epochs 15
```
...
```bash
python CXREncoderTransformer/test_pool_anchor_index.py
python CXREncoderTransformer/verify_collapse_fix.py --max_samples 2000 --mini_steps 80
python CXREncoderTransformer/diagnose_collapse.py --max_samples 3000 --epochs 5
```
...
| [`verify_collapse_fix.py`](verify_collapse_fix.py) | Quick check: multi-class preds + image sensitivity |
| [`diagnose_collapse.py`](diagnose_collapse.py) | Collapse root-cause experiments |
