...
if [[ -n "${SLURM_SUBMIT_DIR}" ]]; then
  ...
  SCRIPT_DIR="${PROJECT_DIR}/baseline2"
else
...
cd "${SCRIPT_DIR}"
python train.py --csv_path "${CSV_PATH}" --cxr_root "${CXR_ROOT}" --metadata_path "${METADATA_PATH}" --ecg_ckpt "${ECG_CKPT}" --output_dir "${OUTPUT_DIR}" "$@"
