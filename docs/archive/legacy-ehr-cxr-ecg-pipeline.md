...
├── legacy/baselines/BaselineExperiment/            # ARDS 三分类 baseline（EHR / CXR / ECG / 多模态）
├── legacy/ehr_temporal/EHRTrend/                      # p2f/s2f 趋势预测、next-step、forward MLP
├── legacy/ehr_temporal/EHRWindowTransformer/          # 基于 anchor + history window 的 Transformer
├── legacy/encoder_transformers/EHREncoderTransformer/         # Symile 预处理 + Row MLP + causal Transformer（单阶段分类）
├── legacy/encoder_transformers/EHREncoderTransformerEmbedPred/  # 上述 + anchor embed 预测（两阶段：预训练 + 微调）
├── legacy/encoder_transformers/CXREncoderTransformer/         # CXR + Transformer（ARDS 变化预测）
├── legacy/encoder_transformers/ECGEncoderTransformer/         # ECG + Transformer
├── legacy/artifacts/figures/                       # 实验对比图、t-SNE 等可视化
├── legacy/artifacts/logs/                          # Slurm 训练日志
├── legacy/shared/models/encoders/               # 共享 EHR / CXR / ECG encoder
└── legacy/early_matching/experiment1_old/              # 旧版 CXR–supertable–waveform 全量匹配（已归档）
```
...
flowchart TD
    ...
    ENRICH --> TREND[legacy/ehr_temporal/EHRTrend/build_trend_dataset.py]
    TREND --> ANCH[legacy/ehr_temporal/EHRTrend/data/ehr_trend_anchors.csv]
...
| 文件 | 行数（约） | 生成脚本 | 含义 |
|------|-----------|----------|------|
| `legacy/ehr_temporal/EHRTrend/data/ehr_trend_anchors.csv` | 118,409 | `legacy/ehr_temporal/EHRTrend/build_trend_dataset.py` | 从 enriched CSV 构建的 **趋势 anchor**：每行含 `subject_id`、`index`、`p2f_vent_fio2`、`prev_state`、`curr_state`、`trend_label`（0=decrease, 1=remain, 2=increase）、`n_window_points` |
...
| 模块 | 主要 CSV |
|------|----------|
| `legacy/baselines/BaselineExperiment/EHRUni` | `p2f_ehr_classified.csv` + `p2f_vent_fio2_enriched.csv`（history） |
| `legacy/baselines/BaselineExperiment/CXRUni` | `p2f_cxr_classified.csv` |
| `legacy/baselines/BaselineExperiment/ECGUni` | `p2f_ecg_all_classified.csv` |
| `legacy/baselines/BaselineExperiment/MultimodalECGCXR` | `p2f_ecg_cxr_multimodal.csv` |
| `legacy/baselines/BaselineExperiment/MultimodalEHRCXR` | `p2f_ehr_classified.csv` + `p2f_cxr_classified.csv` + enriched |
| `legacy/baselines/BaselineExperiment/MultimodalEHRECG` | `p2f_ehr_classified.csv` + `p2f_ecg_all_classified.csv` + enriched |
...
python legacy/ehr_temporal/EHRTrend/build_trend_dataset.py \
  --source_csv data/p2f_vent_fio2_enriched.csv \
  --output_csv legacy/ehr_temporal/EHRTrend/data/ehr_trend_anchors.csv
```
...
## 旧版 pipeline（`legacy/early_matching/experiment1_old/`）

早期实验按 **同一小时** 对齐 CXR + supertable + waveform，与当前 p2f lookback pipeline **逻辑不同**：

| 步骤 | 脚本 | 输出 |
|------|------|------|
| CXR–supertable 匹配 | `legacy/early_matching/experiment1_old/run_full_match.py` | `cxr_supertable_matched.csv` |
| 合并 waveform | `legacy/early_matching/experiment1_old/merge_cxr_waveform.py` | `cxr_supertable_waveform_matched.csv` |

旧 baseline（`legacy/early_matching/experiment1_old/baseline*`）直接使用 `cxr_supertable_waveform_matched.csv`。新实验请使用 `data/p2f_*.csv` 系列。
...
| 模型 | 目录 | 结构要点 | 训练方式 |
|------|------|----------|----------|
| **EHRWindowTransformer** | [`legacy/ehr_temporal/EHRWindowTransformer/`](../../legacy/ehr_temporal/EHRWindowTransformer/) | 原始 EHR 特征 → 直接 Transformer → dual heads | 单阶段 |
| **EHREncoderTransformer** | [`legacy/encoder_transformers/EHREncoderTransformer/`](../../legacy/encoder_transformers/EHREncoderTransformer/) | Symile pct+indicator → Row MLP → causal Transformer → dual heads | 单阶段 |
| **EHREncoderTransformerEmbedPred** | [`legacy/encoder_transformers/EHREncoderTransformerEmbedPred/`](../../legacy/encoder_transformers/EHREncoderTransformerEmbedPred/) | 同上 + 从窗口预测 anchor@t 的 row embedding | **两阶段**：embed 预训练 → cls 微调 |
...
|------|----------------|----------|---------|---------|------|
| **EHRWindowTransformer** | `47103601` / `legacy/ehr_temporal/EHRWindowTransformer/output_direct_window/` | 无 Symile；DirectWindowTransformer | 57.2% | 55.7% | 低于多数类基线；loss ~1.78 |
| **EHREncoderTransformer baseline** | `47745355` / `legacy/encoder_transformers/EHREncoderTransformer/output/` | `class_weights=True`, `p2f_weight=10`, `lr=5e-4` | 44.8% | 46.0% | **loss 卡在 ~1.09**（≈ln3），验证预测单类塌缩 |
| **EmbedPred baseline** | `47748678` / `legacy/encoder_transformers/EHREncoderTransformerEmbedPred/output_twophase/` | 两阶段；预训练 checkpoint 有 bug；`class_weights=True` | 48.9% | 43.4% | 预训练几乎未收敛即进入微调 |
...
**进行中 follow-up**（`bash legacy/artifacts/figures/run_followup_experiments.sh` 批量提交）：
...
| CXR-Fix-A | `47837620` | `legacy/encoder_transformers/CXREncoderTransformer/output_fixA/` | 无 class weights + label smoothing（5k 子集） |
| CXR-Fix-B | `47837621` | `legacy/encoder_transformers/CXREncoderTransformer/output_fixB/` | Fix-A + 全量数据 |
| ECG-Fix-A | `47837622` | `legacy/encoder_transformers/ECGEncoderTransformer/output_fixA/` | `--no-use_class_weights` |
| ECG-Fix-B | `47837623` | `legacy/encoder_transformers/ECGEncoderTransformer/output_fixB/` | Fix-A + `p2f_weight=5`, `early_stop_patience=20` |

> 日志路径：`legacy/artifacts/logs/ehr-*-<jobid>.out`；完整指标见各目录下 `results.json` 与 `classification_report_test.json`。
>
> **自动刷新**（等全部 follow-up job 结束后重跑绘图并打印指标摘要）：
> ```bash
> bash legacy/artifacts/figures/refresh_after_jobs.sh          # 前台等待
> # 或查看后台任务日志：tail -f legacy/artifacts/logs/refresh-after-jobs.log
> python legacy/artifacts/figures/summarize_followup_results.py  # 仅查看当前 results.json 状态
> ```
...
- **验证**：诊断脚本 [`legacy/encoder_transformers/EHREncoderTransformer/diagnose_training.py`](../../legacy/encoder_transformers/EHREncoderTransformer/diagnose_training.py) 实验 E 显示加权 CE ≈1.13、去掉权重后 loss 可降至 ~0.91。

#### 2. EmbedPred 预训练 checkpoint 衔接错误

- **现象**：baseline 预训练在第 1 epoch 就保存 `best.pt`（`val_embed≈0`），微调从近乎随机权重起步。
- **修复**（[`legacy/encoder_transformers/EHREncoderTransformerEmbedPred/train.py`](../../legacy/encoder_transformers/EHREncoderTransformerEmbedPred/train.py)）：
...
```bash
sbatch --job-name=ehr-embed-best legacy/encoder_transformers/EHREncoderTransformerEmbedPred/run_train.sh \
  --output_dir legacy/encoder_transformers/EHREncoderTransformerEmbedPred/output_twophase_expC \
...
Checkpoint：`legacy/encoder_transformers/EHREncoderTransformerEmbedPred/output_twophase_expC/finetune/best_acc.pt`

**EHREncoderTransformer 最佳（Fix-C，单阶段）：**

```bash
sbatch --job-name=ehr-tr-best legacy/encoder_transformers/EHREncoderTransformer/run_train.sh \
  --output_dir legacy/encoder_transformers/EHREncoderTransformer/output_fixC \
  --no-use_class_weights \
  --grad_clip 1.0 \
  --label_smoothing 0.05 \
  --lr 1e-4 \
...
Checkpoint：`legacy/encoder_transformers/EHREncoderTransformer/output_fixC/best_acc.pt`

**快速诊断（~20 分钟）：**

```bash
sbatch legacy/encoder_transformers/EHREncoderTransformer/run_diagnose.sh
# 报告 → legacy/encoder_transformers/EHREncoderTransformer/output_diagnose/report.json
```

**消融批量提交：**

```bash
bash legacy/encoder_transformers/EHREncoderTransformer/run_fix_ablations.sh
```
...
```bash
bash legacy/artifacts/figures/run_all_plots.sh
```

| 子目录 | 脚本 | 内容 |
|--------|------|------|
| [`legacy/artifacts/figures/ehr_embedpred_exp_runs/`](../../legacy/artifacts/figures/ehr_embedpred_exp_runs/) | [`plot_embedpred_exp_runs.py`](../../legacy/artifacts/figures/plot_embedpred_exp_runs.py) | EmbedPred baseline / Exp A–C 曲线、t-SNE、test acc 对比 |
| [`legacy/artifacts/figures/ehr_symile_47745355_47745356_47748678/`](../../legacy/artifacts/figures/ehr_symile_47745355_47745356_47748678/) | [`plot_symile_runs.py`](../../legacy/artifacts/figures/plot_symile_runs.py) | symile-tr / embed / 2ph 对比 |
| [`legacy/artifacts/figures/ehr_tr_fix_runs/`](../../legacy/artifacts/figures/ehr_tr_fix_runs/) | [`plot_ehr_tr_fix_runs.py`](../../legacy/artifacts/figures/plot_ehr_tr_fix_runs.py) | EHREncoderTransformer baseline / Fix-A/C/D/E + t-SNE |
| [`legacy/artifacts/figures/ehr_window_transformer/`](../../legacy/artifacts/figures/ehr_window_transformer/) | [`plot_ehr_window_tr.py`](../../legacy/artifacts/figures/plot_ehr_window_tr.py) | EHRWindowTransformer `47103601` |
| [`legacy/artifacts/figures/cxr_ecg_enc_tr/`](../../legacy/artifacts/figures/cxr_ecg_enc_tr/) | [`plot_cxr_ecg_runs.py`](../../legacy/artifacts/figures/plot_cxr_ecg_runs.py) | CXR `47646619` + CXR/ECG `output/results.json` |

t-SNE（GPU，~30 min）：

```bash
sbatch legacy/artifacts/figures/run_tsne_embedpred.sh    # EmbedPred Exp C vs baseline → ehr_embedpred_exp_runs/
sbatch legacy/artifacts/figures/run_tsne_ehr_tr.sh       # EHREncoderTransformer Fix-C vs baseline → ehr_tr_fix_runs/
```
